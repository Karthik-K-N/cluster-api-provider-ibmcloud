...
type ClientFactory struct {
	...
	AccountFactory            func() (string, error)
	SessionFactory            func() (*ibmpisession.IBMPISession, error)
...
func NewPowerVSClusterScope(params PowerVSClusterScopeParams) (*PowerVSClusterScope, error) { //nolint:gocyclo
	...
	// Fetch user account.
	account, err := params.getAccount(auth)
	if err != nil {
		return nil, fmt.Errorf("failed to get account details %w", err)
	}
	piOptions.UserAccount = account

	// create IBM PowerVS session.
	session, err := params.getPowerVSSession(piOptions.IBMPIOptions)
	if err != nil {
		return nil, fmt.Errorf("failed to get PowerVS session %w", err)
	}
	piOptions.Session = session
...
func (params PowerVSClusterScopeParams) getAccount(auth core.Authenticator) (string, error) {
	if params.AccountFactory != nil {
		return params.AccountFactory()
	}
	return utils.GetAccount(auth)
}

func (params PowerVSClusterScopeParams) getPowerVSSession(options *ibmpisession.IBMPIOptions) (*ibmpisession.IBMPISession, error) {
	if params.SessionFactory != nil {
		return params.SessionFactory()
	}
	return ibmpisession.NewIBMPISession(options)
}
