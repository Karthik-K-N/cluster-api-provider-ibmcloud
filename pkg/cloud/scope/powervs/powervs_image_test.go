...
func TestGetOrImportImage(t *testing.T) {
	...
	existingImages := &models.Images{
		Images: []*models.ImageReference{
			{Name: core.StringPtr("existing-image"), ImageID: core.StringPtr("existing-image-id")},
		},
	}
	otherImages := &models.Images{
		Images: []*models.ImageReference{
			{Name: core.StringPtr("other-image"), ImageID: core.StringPtr("other-image-id")},
		},
	}
	completedJob := &models.Job{
		Status: &models.Status{State: core.StringPtr(string(infrav1.PowerVSImageStateCompleted))},
	}
	failedJob := &models.Job{
		Status: &models.Status{State: core.StringPtr(string(infrav1.PowerVSImageStateFailed))},
	}
	inProgressJob := &models.Job{
		Status: &models.Status{State: core.StringPtr("in-progress")},
	}
	jobRef := &models.JobReference{ID: core.StringPtr("new-job-id")}
