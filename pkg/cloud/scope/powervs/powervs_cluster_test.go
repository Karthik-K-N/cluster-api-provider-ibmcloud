	...
	testCases := []struct {
	...
	}{
		{
			...
			setupScope: func(t *testing.T) *ClusterScope {
				...
				mockVPC.EXPECT().GetLoadBalancerByName(gomock.Any()).Return(&vpcv1.LoadBalancer{
					...
					Name:               ptr.To("test-cluster-lb-private"),
					ProvisioningStatus: ptr.To(string(infrav1.LoadBalancerStateDeletePending)),
				}, nil)
				return &ClusterScope{
					IBMPowerVSCluster: &infrav1.IBMPowerVSCluster{
		...
		},
		{
			...
			setupScope: func(t *testing.T) *ClusterScope {
				...
				mockVPC.EXPECT().GetLoadBalancerByName("test-cluster-lb-public").Return(nil, nil)
				// Second LB (i=1): auto-name with qualifier "1", already gone
				mockVPC.EXPECT().GetLoadBalancerByName("test-cluster-lb-public-1").Return(nil, nil)
				return &ClusterScope{
					IBMPowerVSCluster: &infrav1.IBMPowerVSCluster{
						ObjectMeta: metav1.ObjectMeta{Name: "test-cluster"},
						Spec: infrav1.IBMPowerVSClusterSpec{
