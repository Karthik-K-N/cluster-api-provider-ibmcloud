...
func TestCreateIgnitionData(t *testing.T) {
	...
	t.Run("error when bucket name or region not set in cluster spec", func(t *testing.T) {
		g := NewWithT(t)
		scope := setupPowerVSMachineScope(clusterName, machineName, ptr.To(pvsImage), ptr.To(pvsNetwork), true, nil)
		// COSClient is nil (ignition not configured) and spec has no bucket — hits the spec guard.
...
func TestCreateIgnitionDataAdditionalBranches(t *testing.T) {
	...
	t.Run("error when bucket name or region is not set in cluster spec", func(t *testing.T) {
		g := NewWithT(t)
		setup(t)
		t.Cleanup(teardown)
		...
		scope := setupPowerVSMachineScope(clusterName, machineName, ptr.To(pvsImage), ptr.To(pvsNetwork), true, nil)
		scope.COSClient = mockCOS
		// Spec has no BucketName/BucketRegion set
		scope.IBMPowerVSCluster.Spec.COSInstance = infrav1.COSInstanceSource{}

		_, err := scope.createIgnitionData(ctx, []byte("some-data"))
		g.Expect(err).To(HaveOccurred())
		g.Expect(err.Error()).To(ContainSubstring("COS bucket name or region is not set"))
	})
}
...
func TestResolveUserDataIgnitionPath(t *testing.T) {
	...
	t.Run("error when createIgnitionData fails in ignition path", func(t *testing.T) {
		...
		scope := setupPowerVSMachineScope(clusterName, machineName, ptr.To(pvsImage), ptr.To(pvsNetwork), true, nil)
		// Enable ignition by setting a COS instance on the cluster spec.
		scope.IBMPowerVSCluster.Spec.COSInstance = infrav1.COSInstanceSource{
			Type:         infrav1.SourceTypeProvision,
			BucketName:   "test-bucket",
			BucketRegion: "us-south",
...
func TestIgnitionUserData(t *testing.T) {
	...
	t.Run("error when iamTokenFetcher fails", func(t *testing.T) {
		g := NewWithT(t)
		setup(t)
		t.Cleanup(teardown)

		orig := iamTokenFetcher
		t.Cleanup(func() { iamTokenFetcher = orig })
		iamTokenFetcher = func() (string, error) { return "", errors.New("iam fetch error") }

		mockCOS.EXPECT().PutObject(gomock.Any()).Return(nil, nil)
		scope := makeScope("2.3", mockCOS)
		_, err := scope.ignitionUserData(ctx, []byte("bootstrap"))
		g.Expect(err).To(HaveOccurred())
		g.Expect(err.Error()).To(ContainSubstring("iam fetch error"))
	})

	t.Run("error when IAM token is empty", func(t *testing.T) {
		g := NewWithT(t)
		setup(t)
		t.Cleanup(teardown)
		...
		orig := iamTokenFetcher
		t.Cleanup(func() { iamTokenFetcher = orig })
		iamTokenFetcher = func() (string, error) { return "", nil }

		mockCOS.EXPECT().PutObject(gomock.Any()).Return(nil, nil)
		scope := makeScope("2.3", mockCOS)
		_, err := scope.ignitionUserData(ctx, []byte("bootstrap"))
		g.Expect(err).To(HaveOccurred())
		g.Expect(err.Error()).To(ContainSubstring("IAM token is empty"))
	})
	...
	t.Run("error when ignition version cannot be parsed", func(t *testing.T) {
		...
		orig := iamTokenFetcher
		t.Cleanup(func() { iamTokenFetcher = orig })
		iamTokenFetcher = func() (string, error) { return "tok", nil }
	...
	t.Run("produces v2 ignition redirect document", func(t *testing.T) {
		...
		orig := iamTokenFetcher
		t.Cleanup(func() { iamTokenFetcher = orig })
		iamTokenFetcher = func() (string, error) { return "mytoken", nil }
		...
		g.Expect(string(data)).To(ContainSubstring("Bearer mytoken"))
	})
	...
	t.Run("produces v3 ignition redirect document", func(t *testing.T) {
		...
		orig := iamTokenFetcher
		t.Cleanup(func() { iamTokenFetcher = orig })
		iamTokenFetcher = func() (string, error) { return "mytoken3", nil }
		...
		g.Expect(string(data)).To(ContainSubstring("Bearer mytoken3"))
	})
...
func TestResolveUserDataIgnitionSuccess(t *testing.T) {
	t.Run("returns base64-encoded ignition v2 document when ignition is configured", func(t *testing.T) {
		...
		orig := iamTokenFetcher
		t.Cleanup(func() { iamTokenFetcher = orig })
		iamTokenFetcher = func() (string, error) { return "sometoken", nil }
